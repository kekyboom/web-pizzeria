...
function CardPizza ({imageSrc, title, description, price}) {
    return (
        <>
            <div className="bg-white border rounded-lg shadow-lg overflow-hidden dark:bg-gray-50 w-80 h-max">
                {/* Imagen*/}
                <img className="w-full object-cover h-52" src={imageSrc} alt={title} />

                {/* Contenido*/}
                <div className="p-5 flex flex-col items-center text-center">

                    <h5 className="mb-3 text-2xl font-semibold text-gray-900 dark:text-black">{title}</h5>
                    <hr className="border-t-2 border-gray w-full pb-2" />
                    <p className="text-black pb-3">Ingredientes:</p>

                    <div className="flex ">
                        <LuPizza className="text-black "/>
                        <p className="pl-1 mb-3 text-sm text-gray-700 dark:text-gray-700">{description}</p>
                    </div>

                    <p className="mb-5 text-lg font-bold text-gray-900 dark:text-black">Precio: ${price}</p>
